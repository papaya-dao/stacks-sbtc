(define-constant err-not-a-peg-wallet (err u501)) ;; There is no peg wallet address for the specified wallet.
(define-constant err-invalid-spending-pubkey (err u503)) ;; The recipient of the BTC is not the same as the pubkey that unlocked the spending script.
(define-constant err-peg-value-not-found (err u505)) ;; There was no output containing the peg wallet scriptPubKey.
(define-constant err-missing-witness (err u506)) ;; The Taproot witness was missing.
(define-constant err-unlock-script-not-found-or-invalid (err u507)) ;; The unlock script at the specified witness index did not exist or was invalid. (Not according to the sBTC spec.)

(define-constant err-script-invalid-opcode (err u510)) ;; The opcode in the unlock script was invalid.
(define-constant err-script-invalid-version (err u511)) ;; The version in the unlock script was invalid.
(define-constant err-script-not-op-drop (err u512)) ;; The script does not contain OP_DROP at the expected offset.
(define-constant err-script-checksig-missing (err u513)) ;; The script does not contain OP_CHECKSIG at the expected offset.
(define-constant err-script-missing-pubkey (err u514)) ;; The script does not contain a Taproot pubkey.
(define-constant err-script-invalid-principal (err u515)) ;; The encoded Stacks principal inside the script is invalid.
(define-constant err-script-invalid-length (err u516)) ;; The length of the script is different from what is expected.
