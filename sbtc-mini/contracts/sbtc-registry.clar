...
(define-constant err-burn-tx-already-processed (err u600)) ;; A burnchain TXID was processed (seen) before.
(define-constant err-peg-wallet-already-set (err u602)) ;; A peg wallet address for the specified cycle was already set.
(define-constant err-minimum-burnchain-confirmations-not-reached (err u603)) ;; The burnchain transaction did not yet reach the minimum amount of confirmation.
(define-constant err-not-settled-state (err u604)) ;; The state passed to function `get-and-settle-pending-peg-out-request` was not a settled state. (Fulfilled or cancelled.)
(define-constant err-invalid-txid-length (err u605)) ;; The passed TXID byte length was not equal to 32.
(define-constant err-unknown-peg-out-request (err u606)) ;; The peg-out request ID passed to `get-and-settle-pending-peg-out-request` does not exist.
(define-constant err-peg-out-not-pending (err u607)) ;; The peg-out request ID passed to `get-and-settle-pending-peg-out-request` is not in a pending state.
