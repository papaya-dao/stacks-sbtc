...
(define-constant ERR_MISSING_DISBURSEMENTS u1)
(define-constant ERR_TX_ALREADY_PROCESSED u2)
(define-constant ERR_INVALID_BITCOIN_TX u3)
(define-constant ERR_INVALID_DISBURSEMENT_HASH u4)

(define-constant ERR_NOT_REGISTRATION_WINDOW u1000)
(define-constant ERR_ALREADY_REGISTERED u1001)
(define-constant ERR_SBTC_NOT_AUTHORIZED_STACKER_DELEGATE u1002)
(define-constant ERR_INSUFFICIENT_UNLOCK_BALANCE u1003)
(define-constant ERR_SIGNING_KEY_ALREADY_USED u1004)
(define-constant ERR_NOT_A_STACKER u1005)
(define-constant ERR_STACKER_DOES_NOT_UNLOCK u1005)
(define-constant ERR_STACKER_NOT_QUALIFIED u1006)
(define-constant ERR_STACKER_ALREADY_PRE_REGISTERED u1007)
(define-constant ERR_PAYOUT_ADDRESS_ALREADY_USED u1008)

(define-constant ERR_NOT_VOTING_WINDOW u2000)
(define-constant ERR_SIGNER_NOT_REGISTERED u2002)
(define-constant ERR_SIGNER_ALREADY_VOTED u2003)
(define-constant ERR_NOT_ENOUGH_STX u2004)

(define-constant ERR_NO_WINNING_POX_ADDR u3000)
(define-constant ERR_FUTURE_REWARD_CYCLE u3001)
(define-constant ERR_PAYOUT_NOT_CALCULATED u3002)
(define-constant ERR_PAYOUT_ALREADY_MADE u3003)
(define-constant ERR_NOT_A_SIGNER u3004)
...
(define-read-only (is-authorized-as-stacker (stacker principal)
                                            (burn-block-ht uint))
    (match (contract-call? .pox-2 get-allowance-contract-callers stacker (as-contract tx-sender))
        allowance-record
...
(define-read-only (get-next-btc-payout (rc uint) (burn-ht uint))
    (let (
        ...
        (match pox-rec-opt
            ...
            ;; have not started
            (let (
                (winning-pox-addr (unwrap! (map-get? winning-pox-addrs rc) (err ERR_NO_WINNING_POX_ADDR)))
            )
...
(define-read-only (get-btc-owed (rc uint) (signer principal))
    (let (
        ...
        (signer-rec (unwrap! (map-get? signer-state { signer: signer, reward-cycle: rc })
            (err ERR_NOT_A_SIGNER)))
        (tabulation (unwrap! (map-get? disbursed-btc-state rc)
            (err ERR_PAYOUT_NOT_CALCULATED)))
    )
...
(define-read-only (decode-disbursement-tx (tx (buff 1024)) (reward-cycle uint))
    (let (
        ...
        (asserts! (is-eq (sha512/256 (concat dh btc-block-hash)) op-return-dh)
            (err ERR_INVALID_DISBURSEMENT_HASH))
    ...
    (let (
        ...
        (asserts!
            (match cur-signer-rec-opt
                ...
                ;; not registered
                true)
            (err ERR_STACKER_NOT_QUALIFIED))

        ;; signer has enough STX
        (asserts! (>= (get unlocked account) amount-ustx)
            (err ERR_INSUFFICIENT_UNLOCK_BALANCE))
...
(define-read-only (can-signer-register (signer principal)
    ...
    (let (
        ...
        (asserts! (is-none (map-get? signer-public-keys-used public-key))
            (err ERR_SIGNING_KEY_ALREADY_USED))
        ...
        (asserts!
            (match (map-get? payout-addrs-in-reward-cycle payout-addr)
                ...
                true)
            (err ERR_PAYOUT_ADDRESS_ALREADY_USED))
...
(define-read-only (inner-get-sbtc-wallet-addr (burn-ht uint))
    (let (
        ...
        (if (or (in-transfer-window burn-ht) (in-penalty-window burn-ht))
...
(define-private (stack-new-signer-ustx (signer principal)
    ...
    (let (
        ...
        (unwrap-panic (contract-call? .pox-2 delegate-stx amount-ustx (as-contract tx-sender) (some until-burn-ht) (some pox-addr)))
        
        ;; have the contract go stack them
        (as-contract
            (unwrap-panic
                (contract-call? .pox-2 delegate-stack-stx signer amount-ustx pox-addr cur-burn-ht u1)))
        ...
        (match pre-register-rec-opt
            ...
            pre-register-rec
                (begin
                   (as-contract
                       (unwrap-panic
                           (contract-call? .pox-2 stack-aggregation-increase pox-addr rc (unwrap-panic (get agg-index pre-register-rec)))))
                    pre-register-rec
                )
            ...
            ;; no PoX record, so go and aggregate-commit for this new one 
            {
                agg-index: (some (as-contract
                    (unwrap-panic (contract-call? .pox-2 stack-aggregation-commit-indexed pox-addr rc)))),
                total-ustx: u0,
                num-signers: u0
            })
    ))
...
(define-private (stack-returning-signer-ustx (signer principal)
    ...
    (let (
        ...
        (as-contract
            (unwrap-panic
                (contract-call? .pox-2 delegate-stack-extend signer pox-addr u1)))
        ...
        (match vote-rec-opt
            ...
            vote-rec
                (begin
                    (as-contract
                        (unwrap-panic
                            (contract-call? .pox-2 stack-aggregation-increase pox-addr rc (unwrap-panic (get agg-index vote-rec)))))
                    {
