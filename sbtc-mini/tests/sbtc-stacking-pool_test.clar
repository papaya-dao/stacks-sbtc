...
;; @name Set protocol caller test
(define-public (test-is-protocol-caller)
	(begin 
		(unwrap! (contract-call? .pox-3 allow-contract-caller .sbtc-stacking-pool none) (err u1))
		;; @mine-empty 1
		(ok true)
	)
)
...
;; 	(begin
;; 		(unwrap! (contract-call? .sbtc-stacking-pool get-current-window) (err u1))
;; 		(ok true)
;; 	)
;; )

;;(unwrap! (contract-call? .sbtc-stacking-pool is-protocol-caller) (err u2))
;; @name Querying volunteer can pre-register in cycle (n - 1) to register in cycle n
;; @mine-blocks-before 70200
;; (define-public (test-pre-register)
;; 	(begin
;; 		(unwrap! (contract-call? .pox-3 allow-contract-caller .sbtc-stacking-pool none) (err u1))
;; 		;; @mine-empty 70200
;; 		;;(unwrap! (contract-call? .sbtc-stacking-pool signer-pre-register u100000000000 { version: 0x06, hashbytes: 0x0011223344556699001122334455669900112233445566990011223344556699 }) (err u2))
;; 		;; (unwrap!
